  ...
  convertYarnToNpmV2(yarnLock, name, version, packageDir, packageJson) {
    ...
    const flattenDeps = (_tree, prefix = '') => _tree
      ? Object.keys(_tree).reduce((tree, name) => {
        ...
        return {
          ...tree,
          ...flattenDeps(entry.dependencies, prefix + `node_modules/${name}/`),
          [prefix + name]: {
            ...pick(entry, 'version', 'resolved', 'integrity', 'license'),
    ...
    const packages = flattenDeps(dependencies)

    // append root package
    // packages[""] = pick(packageJson, 'name', 'version', 'license', 'workspaces', 'dependencies')

...
function createPackageLockTree ({physicalTree}) {
  ...
  const fileVersion = /^file:/.test(logicalNode.version)
    ? logicalNode.version
    : false
  const githubVersion = /^git+/.test(logicalNode.resolved)
    ? logicalNode.resolved
    : false
  return {
    bundled: physicalTree.bundled || undefined,
    optional: physicalTree.optional || undefined,
    dev: physicalTree.dev || undefined,
    version: githubVersion || fileVersion || physicalTree.version,
    resolved: physicalTree.bundled || githubVersion || fileVersion
      ? undefined
      : physicalTree.manifest._resolved,
    integrity: physicalTree.bundled || githubVersion || fileVersion
      ? undefined

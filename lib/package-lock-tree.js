...
function packageLockTree ({physicalTree, logicalTree, parent = {}}) {
  ...
  const logicalNode = physicalTree.isRoot
    ? logicalTree
    : logicalTree.nodes[physicalTree.manifest._id]
  const state = {
    optional: physicalTree.isOptional,
    bundled: !!physicalTree.bundled || undefined,
    name: physicalTree.manifest.name,
    version: physicalTree.manifest.version,
    ...
    children: physicalTree.children.length > 0
      ? physicalTree.children.map(c => {
        return packageLockTree({physicalTree: c, logicalTree, parent: self})
      })
      : []
  }
  return self

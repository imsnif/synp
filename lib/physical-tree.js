...
function nmPhysicalTree ({bundled, optional, unlinked, logicalNode = {}, parent = false}) {
  const self = Object.freeze({
    ...
    addChild (logicalNode) {
      const { name, version, children, bin } = logicalNode
    ...
    addChildren (children) {
      const treeRoot = self.root
      const logicalDescendants = Object.keys(children)
        .sort()
        .reduce((logicalDescendants, childName) => {
          const logicalNode = children[childName]
          const child = self.addChild(logicalNode)
          if (!self.unlinked) {
            logicalDescendants.push({child, children: logicalNode.children})
          }

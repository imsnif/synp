...
const { formatDependenciesForYarn, formatYarnResolved } = require('../util/format')
...
function createEntry (key, manifest, node, entries) {
  ...
  const { deps, optDeps } = formatDependenciesForYarn(
  ...
  entry.data = {
    ...
    dependencies: Object.keys(deps).length > 0 ? deps : null,
    optionalDependencies: Object.keys(optDeps).length > 0 ? optDeps : null,
